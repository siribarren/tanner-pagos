...
class EmailService:
    def enviar_compromiso_creado(self, destinatario: str, fecha_compromiso: date, monto: int) -> None:
        if not destinatario:
        ...
        cuerpo = (
            f"Hola,\n\n"
            f"Se acaba de crear un compromiso de pago por {self.formato_monto(monto)} "
            f"con fecha {self.formato_fecha(fecha_compromiso)}.\n\n"
    ...
    def formato_fecha(self, fecha: date) -> str:
        return f"{fecha.day} de {MESES_ES[fecha.month - 1]} de {fecha.year}"

    def formato_monto(self, monto: int) -> str:
