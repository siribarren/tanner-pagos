...
def get_docai_config() -> DocAiConfig:
    ...
    assert project_id, "La variable de entorno GCP_PROJECT_ID no está configurada."

    location = environ.get("DOCAI_LOCATION")
    assert location in ("us", "eu"), "DOCAI_LOCATION debe ser 'us' o 'eu'."

    ocr_processor = environ.get("DOCAI_PROCESSOR_ID")
    assert ocr_processor, "La variable de entorno DOCAI_PROCESSOR_ID no está configurada."
